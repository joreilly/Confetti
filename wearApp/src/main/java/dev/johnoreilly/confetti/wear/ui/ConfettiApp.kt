...
@Composable
fun ConfettiApp(
    component: DefaultWearAppComponent,
        ...
        ConfettiTheme(settings.theme) {
            SwipeToDismissBox(
                ...
                when (val child = configuration?.instance) {
                    is WearAppComponent.Child.Conferences -> ConferencesRoute(child.component, ScalingLazyColumnDefaults.belowTimeText().create())
                    is WearAppComponent.Child.ConferenceSessions -> SessionsRoute(child.component, ScalingLazyColumnDefaults.belowTimeText().create())
                    else -> Text(
                        text = "Configuration\n${
