...
actual fun getEmbeddingCache(): EmbeddingCache? = runCatching {
    val context: android.content.Context = org.koin.core.context.GlobalContext.get().get()
    val root = okio.Path.Companion.run {
        "${context.cacheDir.absolutePath}/confetti-embeddings".toPath()
    }
    EmbeddingCache(okio.FileSystem.SYSTEM, root)
}.getOrNull()
