...
tasks.register("setupCredentials") {
    doLast {
        if (System.getenv("CI")?.isNotEmpty() == true) {
            ...
            file("backend/gcp_service_account_key.json").writeText(System.getenv("GOOGLE_SERVICES_JSON"))
            file("backend/firebase_service_account_key.json").writeText(System.getenv("FIREBASE_SERVICES_JSON"))
