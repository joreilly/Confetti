...
class ConferenceAppFunctions : KoinComponent, ConferenceInfoSchemaDefinition, ConferenceSessionsSchemaDefinition,
    ...
    override suspend fun conferenceInfo(appFunctionContext: AppFunctionContext): AppFunctionConference? {
        ...
        val details =
            confettiRepository.conferenceData(conference, fetchPolicy = FetchPolicy.CacheOnly).data ?: return null

        return AppFunctionConference(details.config.id, details.config.name, details.config.days.map { it.toJavaLocalDate().atStartOfDay() })
    }
    ...
    override suspend fun findSessions(
        ...
        val conference = confettiRepository.getConference()
        val sessions = confettiRepository.sessions(conference, null, null, FetchPolicy.CacheOnly).data?.sessions?.nodes?.map { it.sessionDetails } ?: return null
    ...
    override suspend fun listSpeakers(appFunctionContext: AppFunctionContext): List<AppFunctionSpeaker>? {
        val conference = confettiRepository.getConference()
        val results =
            confettiRepository.conferenceData(conference, fetchPolicy = FetchPolicy.CacheOnly).data?.speakers?.nodes?.map { it.speakerDetails } ?: return null
